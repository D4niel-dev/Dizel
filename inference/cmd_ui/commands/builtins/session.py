...
class SessionCommand(Command):
    ...
    def _render_current(self, app: Any) -> str:
        return (
            f"Current session: {app.session_id or 'None'}\n"
            "Actions: new, list, open <id>, rename <title>, pin, delete [id]"
        )
    ...
    def _render_list(self, app: Any) -> str:
        ...
        for session in sessions[:15]:
            active = ">" if session["id"] == app.session_id else " "
            pinned = "*" if session.get("pinned") else " "
            lines.append(f"  {active}{pinned} {session['id'][:8]} | {session['title'][:36]}")
        lines.append("")
